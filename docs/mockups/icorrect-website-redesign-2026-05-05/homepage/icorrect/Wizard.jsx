  ...
  return (
    <div className="wiz-step wiz-quote">
      ...
      <form className="wiz-contact" onSubmit={submit}>
        <div className="wiz-section-label">Would you like us to collect your MacBook?</div>

        {/* Postcode-first: ask for postcode, then reveal the services available to that area. */}
        <div className="wiz-postcode-card">
          ...
          <div className="wiz-postcode-row">
            ...
            />
            {postcodeKnown && (
              <span className={"wiz-postcode-pill wiz-postcode-pill-" + zone}>
                {zone === "london" ? "London · all 3 services" : "UK · 2 services"}
              </span>
            )}
          </div>
          {zone === "international" && (
        ...
        {/* Estimated dates — shown for all three services once the user has picked one. */}
        {((delivery === "walkin" && walkInDate) || (delivery === "courier" && londonOk) || delivery === "mail") && (
          <div className="wiz-eta">
            {delivery === "walkin" && walkInDate && (
              <div className="wiz-eta-row">
                <span className="wiz-eta-label">Drop-off</span>
                <span className="wiz-eta-value">
                  {fmtDate(walkInDate)}{walkInTime ? ` · ${walkInTime}` : ""}
                </span>
              </div>
            )}
            {delivery === "courier" && londonOk && courierDate && (
              <div className="wiz-eta-row">
                <span className="wiz-eta-label">Collection</span>
                <span className="wiz-eta-value">
                  {fmtDate(courierDate)}{courierWindow ? ` · ${COURIER_WINDOWS.find(w => w.id === courierWindow)?.label}` : ""}
                </span>
              </div>
            )}
            {delivery === "mail" && (
              <React.Fragment>
                <div className="wiz-eta-row">
                  ...
                  <span className="wiz-eta-value">{`Post by ${fmtDate(today)}`}</span>
                </div>
                <div className="wiz-eta-row">
              ...
              >
                ...
                {standardReturnDate && (
                  <div className="wiz-turnaround-eta">
                    <span className="wiz-turnaround-eta-label">Back by</span>
