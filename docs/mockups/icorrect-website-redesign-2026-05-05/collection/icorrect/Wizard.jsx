...
const UK_BANK_HOLIDAYS = new Set([
  ...
  "2026-12-28", // Boxing Day (substitute, since 26 Dec is Sat)
...
function buildTimeline({ delivery, walkInDate, walkInTime, courierDate, courierWindow, arrivalDate, today, returnDate, turnaround }) {
  const steps = [];
  ...
  if (returnDate) {
    steps.push({
      ...
      note: delivery === "walkin"
        ? "We'll text when it's ready — pop in any time before 6pm."
        : "Tracked return delivery, signed-for.",
      final: true,
    });
  }
...
function StepQuote({ model, issue, price, requiresDiagnosis, contact, setContact, delivery, setDelivery, address, setAddress, onBack, onReset }) {
  ...
  const effectiveDelivery =
    delivery === "courier" && postcodeKnown && !londonOk ? "mail" : delivery;
  ...
  const collectionDate = effectiveDelivery === "courier"
    ? (courierDate ? new Date(courierDate) : nextWorkingDay(today))
    : null;
  ...
  const canSubmit = !!delivery
    && (delivery !== "courier" || (londonOk && courierDate && courierWindow))
    ...
    const copy =
      secondary === "email" ? {
        ...
        sub: effectiveDelivery === "walkin"
          ...
          ? `Your London courier will collect ${collectionDate ? fmtDate(collectionDate) : "tomorrow"}${courierWindow ? ` between ${COURIER_WINDOWS.find(w => w.id === courierWindow)?.label}` : ""}.`
  ...
  return (
    <div className="wiz-step wiz-quote">
      ...
      <form className="wiz-contact" onSubmit={submit}>
        {/* Postcode-first: ask for postcode, then reveal the services available to that area. */}
        <div className={"wiz-postcode-card" + (postcodeKnown ? " wiz-postcode-card-resolved" : "")}>
          <label className="wiz-postcode-label" htmlFor="wiz-postcode">
            Enter your postcode
            <span className="wiz-postcode-hint">We'll show you what's available in your area.</span>
          </label>
          <div className="wiz-postcode-row">
            <input
              id="wiz-postcode"
          ...
          {zone === "international" && (
            <div className="wiz-postcode-warn">
              <strong>Looks like a non-UK postcode.</strong> We currently ship within the UK only — you can still <button type="button" className="wiz-postcode-link" onClick={() => { setAddress({ ...address, postcode: "" }); setDelivery("walkin"); }}>walk in to our Fitzrovia workshop</button>.
            </div>
          )}
        </div>

        {/* Services: only shown once a UK postcode is recognised. */}
        {postcodeKnown && (
        ...
        {/* Courier: London-only — pick a collection date + 3-hour window. Address captured at checkout. */}
        {delivery === "courier" && londonOk && (
          <div className="wiz-delivery-detail">
            <div className="wiz-courier-confirm">
              ✓ Same-day courier available for <strong>{(address?.postcode || "").toUpperCase()}</strong> · £{COURIER_FEE} · full address at checkout.
            </div>
            <div className="wiz-section-label">Pick a collection day</div>
            <div className="wiz-walkin-dates">
              {upcomingWalkInDates(8).map((d, i) => {
            ...
            {courierDate && (
              <React.Fragment>
                <div className="wiz-section-label wiz-section-label-sub">Pick a 3-hour collection window</div>
                <div className="wiz-courier-windows">
                  {COURIER_WINDOWS.map(w => (
                    <button
                      key={w.id}
                      type="button"
        ...
        {/* Repair turnaround upsell — only after postcode is recognised */}
        {!requiresDiagnosis && price && postcodeKnown && (
          <div className="wiz-turnaround">
            ...
            <div className="wiz-turnaround-grid">
              ...
              >
                ...
                {standardReturnDate && (
                  <div className="wiz-turnaround-eta">
                    <span className="wiz-turnaround-eta-label">Ready by</span>
                    <span className="wiz-turnaround-eta-date">{fmtDate(standardReturnDate)}</span>
                  </div>
                )}
              </button>
              <button
              ...
              >
                ...
                <span className="wiz-turnaround-price-corner">+£{FAST_TURNAROUND_FEE}</span>
                <div className="wiz-turnaround-title">Fast</div>
                <div className="wiz-turnaround-meta">Same or next working day</div>
                {fastReturnDate && (
                  <div className="wiz-turnaround-eta">
                    <span className="wiz-turnaround-eta-label">Ready by</span>
                    <span className="wiz-turnaround-eta-date">{fmtDate(fastReturnDate)}</span>
                  </div>
                )}
              </button>
            </div>
        ...
        {/* Stepped service timeline — concrete dates so the customer trusts the promise. */}
        {!requiresDiagnosis && price && postcodeKnown && returnDate && (
          (delivery === "walkin" && walkInDate) ||
          (delivery === "courier" && londonOk && courierDate) ||
          delivery === "mail"
        ) ? (
          <div className="wiz-timeline" role="list" aria-label="Repair timeline">
            <div className="wiz-timeline-head">
              <div className="wiz-section-label">Your repair timeline</div>
              <div className="wiz-timeline-promise">Booking reserves your part and the workshop slot — these dates become a guarantee.</div>
            </div>
            <ol className="wiz-timeline-steps">
              {buildTimeline({
                delivery,
                walkInDate, walkInTime,
                courierDate, courierWindow,
                arrivalDate,
                today,
                returnDate,
                turnaround,
                ...
                <li
                  ...
                  role="listitem"
                >
                  <div className="wiz-timeline-marker" aria-hidden="true">
                    {step.final ? "✓" : i + 1}
                  </div>
                  <div className="wiz-timeline-body">
                    <div className="wiz-timeline-label">{step.label}</div>
                    <div className="wiz-timeline-date">{step.date}</div>
                    {step.note && <div className="wiz-timeline-note">{step.note}</div>}
                  </div>
                </li>
              ))}
            </ol>
        ...
        <button
          type="submit"
          disabled={!canSubmit}
          className={"btn btn-lg " + (postcodeKnown ? "btn-dark" : "btn-prompt")}
