...
function Nav() {
  ...
  const links = [
    ["MacBook Repairs.html", "Repairs"],
    ["Corporate Services.html", "Corporate"],
    ["Why Us.html", "Why us"],
  ...
  return (
    <nav className={"icnav" + (menuOpen ? " icnav-open" : "")}>
      <div className="container icnav-inner">
        <a className="icnav-logo" href="iCorrect Homepage.html">iCorrect</a>
        <div className="icnav-links">
          {links.map(([href, label]) => <a key={href} href={href}>{label}</a>)}
        </div>
        <span className="icnav-status"><span className="icnav-status-dot" /> Open · Fitzrovia</span>
        <a href="iCorrect Homepage.html#wizard" className="icnav-cta">Get a quote <span aria-hidden="true">→</span></a>
        <button type="button" className="icnav-burger" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen(o => !o)}>
          <span className="icnav-burger-bar" />
          <span className="icnav-burger-bar" />
          <span className="icnav-burger-bar" />
        </button>
      </div>

      <div className="icnav-mobile" hidden={!menuOpen}>
        <div className="container">
          {links.map(([href, label]) => <a key={href} href={href} onClick={() => setMenuOpen(false)}>{label}</a>)}
          <a href="iCorrect Homepage.html#wizard" className="icnav-mobile-cta" onClick={() => setMenuOpen(false)}>Get a quote <span aria-hidden="true">→</span></a>
          <div className="icnav-mobile-foot">
            <span className="icnav-status-dot" /> Open · Fitzrovia
            <a href="tel:+442070998517">+44 (0)207 099 8517</a>
          </div>
        </div>
      </div>
      {menuOpen && <div className="icnav-scrim" onClick={() => setMenuOpen(false)} />}
    </nav>);
  ...
  return (
    <div className="stars" aria-label={`${rating} out of 5`}>
      {[0, 1, 2, 3, 4].map((i) => {
        ...
        return (
          <svg key={i} width="14" height="14" viewBox="0 0 14 14" style={{ flexShrink: 0, fill: "rgb(245, 166, 35)", stroke: "rgb(245, 166, 35)" }}>
            <defs><linearGradient id={`g${i}`}><stop offset={`${fill * 100}%`} stopColor="#171717" /><stop offset={`${fill * 100}%`} stopColor="#e5e5e5" /></linearGradient></defs>
            <path d="M7 1l1.8 3.8 4.2.6-3 2.9.7 4.1L7 10.5 3.3 12.4l.7-4.1-3-2.9 4.2-.6z" fill={`url(#g${i})`} style={{ fill: "rgb(245, 166, 35)", stroke: "rgb(245, 166, 35)" }} />
          </svg>);
    ...
    <section id="identify" className="identify">
      <div className="container identify-grid">
        ...
        <div className="identify-text">
          ...
          <p>Flip your MacBook over and look for the four-character code starting with "A" near the hinge — or read the longer serial number from the same line. Enter either and we'll match the repair exactly.</p>
          <form className="identify-form" onSubmit={(e) => e.preventDefault()}>
            <input placeholder="Model code (A2442) or serial number" value={input} onChange={(e) => setInput(e.target.value)} />
    ...
    <section className="product-grid">
      <div className="container">
        ...
        <div className="pg">
          ...
          <a key={i} className="pg-item" href="MacBook Screen Repair - Product Page.html">
      ...
      <div className="container icfooter-inner">
        ...
        <div className="icfooter-cols">
          ...
          <div>
            ...
            <a href="Why Us.html">Why us</a><a href="Corporate Services.html">Corporate</a><a href="How It Works.html">How it works</a><a href="Repair Case Study.html">Case study</a><a href="Careers.html">Careers</a><a href="Contact.html">Contact</a>
          </div>
          <div>
