  ...
  return (
    <div className="container">
      <div className="crumbs">
        ...
        <a href="MacBook Repairs.html">Repairs</a>
        <span className="sep">/</span>
        <a href="MacBook Screen Collection.html">MacBook</a>
        <span className="sep">/</span>
        <span>Screen</span>
      </div>
    </div>);
...
function ProductHero({ placement = "top" }) {
  const anchors = [
  { num: "01", label: "What's actually broken", href: "#failure" },
      ...
      <div className="container">
        <div className="product-hero-grid">
          ...
          <div className="product-hero-right">
              <div className="ph-trustcard"><TrustCard /></div>
              <ProductConfigurator />
            </div>
          }
        </div>
...
// Collapsed from 6 detailed cards to 2 buckets — most visitors skim, and the
// detail belongs in the diagnosis flow not the marketing page.
function FailureSection() {
  const buckets = [
    {
      tag: "Bucket 1",
      symptoms: ["Cracked or shattered glass", "Dead pixels, bright spots", "Liquid damage"],
      title: "The panel itself is gone.",
      verdict: "New display assembly.",
      from: "From £699",
    },
    {
      tag: "Bucket 2",
      symptoms: ["Black screen, fan still on", "Vertical bars, flickering", "Dim or uneven backlight"],
      title: "Your panel is fine. Something else failed.",
      verdict: "Board-level repair.",
      from: "From £349",
    },
  ];
  ...
  return (
    <section className="journey" id="failure">
      <div className="container">
        ...
        <h2>Most screen issues fall into two buckets.</h2>
        <p className="journey-lede">
          Half the Macs that come in don't need a new screen at all. We probe every machine under
          the microscope before quoting — so you only pay for what's actually failed.
        </p>

        <div className="failure-buckets">
          {buckets.map((b, i) => (
            <div key={i} className="failure-bucket">
              <div className="failure-bucket-tag">{b.tag}</div>
              <ul className="failure-bucket-symptoms">
                {b.symptoms.map(s => <li key={s}>{s}</li>)}
              </ul>
              <div className="failure-bucket-verdict">
                <h3>{b.title}</h3>
                <div className="failure-bucket-verdict-row">
                  <span className="failure-bucket-fix">{b.verdict}</span>
                  <span className="failure-bucket-from">{b.from}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="failure-callout">
          ...
          <div className="failure-callout-body">
            <strong>Free diagnosis, written quote.</strong> Drop in or post your Mac. We'll tell you which
            bucket — and exactly what it'll cost — before opening anything.
          </div>
        </div>
      </div>
    </section>);
...
function ProcessSection({ videoMode = "inline", processLayout = "detailed" }) {
  const steps = [
  {
    num: "01",
    ...
    short: "Probe the panel and board under microscope. Written quote before anything is opened.",
  ...
  },
  {
    ...
    short: "Done in Fitzrovia, never sent out. Microsoldering at 40× magnification.",
  ...
  },
  {
    ...
    short: "True-Tone calibrated against a reference sensor. 30-point QA pass.",
  ...
  },
  {
    ...
    short: "Collect Mon–Fri or insured next-day courier. QA report and warranty card included.",
    meta: [["Return", "Same/next day"], ["Warranty", "2 years"]],
    video: null
  }];


  if (processLayout === "compact") {
    return (
      <section className="journey" id="process">
        <div className="container">
          ...
          <h2>Drop-off to "fixed".</h2>

          <ol className="process-compact">
            {steps.map((s) => (
              <li key={s.num} className="process-compact-row">
                <span className="process-compact-num">{s.num}</span>
                <div className="process-compact-body">
                  <span className="process-compact-title">{s.title}.</span>
                  <span className="process-compact-desc"> {s.short}</span>
                </div>
                <span className="process-compact-meta">{s.meta[0][1]}</span>
              </li>
            ))}
          </ol>
        </div>
      ...
      <section className="journey" id="process">
        <div className="container">
          ...
          <div className="process-grid">
            {steps.map((s) => (
              <article key={s.num} className="process-card">
                <div className="process-card-head">
                  <span className="process-card-num">{s.num}</span>
                  <span className="process-card-time">{s.meta[0][1]}</span>
                </div>
                <h3 className="process-card-title">{s.title}</h3>
                <p className="process-card-desc">{s.short}</p>
                {s.video ? (
                  <div className="process-card-video">
                    <div className="process-card-video-thumb">
                      <PlaceholderVideoIcon />
                      <div className="process-card-video-play">
                        <svg viewBox="0 0 12 12"><path d="M2 1l9 5-9 5z" /></svg>
                      </div>
                    </div>
                    <div className="process-card-video-meta">
                      <span className="process-card-video-tag">{s.video.tag}</span>
                      <span className="process-card-video-dur">{s.video.dur}</span>
                    </div>
                  </div>
                ) : (
                  <div className="process-card-nodvideo">
                    <span className="process-card-nodvideo-key">Warranty</span>
                    <span className="process-card-nodvideo-val">{s.meta[1][1]}</span>
                  </div>
                )}
              </article>
            ))}
          ...
          <div className="reviews-wall-stat">
            <div className="reviews-wall-stat-row">
              ...
              <div className="reviews-wall-stat-stars">
                {[0, 1, 2, 3, 4].map((i) =>
                <svg key={i} viewBox="0 0 14 14" width="16" height="16"><path d="M7 1l1.8 3.8 4.2.6-3 2.9.7 4.1L7 10.5 3.3 12.4l.7-4.1-3-2.9 4.2-.6z" fill="#f5a623" /></svg>
                )}
              </div>
            </div>
          ...
          <article key={i} className="review-card">
              <div className="review-card-top">
                ...
                <svg key={j} viewBox="0 0 14 14" width="11" height="11">
                      <path d="M7 1l1.8 3.8 4.2.6-3 2.9.7 4.1L7 10.5 3.3 12.4l.7-4.1-3-2.9 4.2-.6z" fill="#f5a623" />
                    </svg>
                )}
                </div>
              </div>
    ...
    <section className="journey eng-brief" id="brief">
      <div className="container">
        ...
        <div className="eng-brief-foot">
          ...
          <a href="Board-Level Diagnosis.html" className="eng-brief-cta">Get a free diagnosis →</a>
        </div>
      </div>
    </section>);
...
// =========================================================================
// NEW — committed-buyer flow sections (Full Product Page).
// Kept separate from FailureSection / PartsSection so the older prototype
// page is untouched. These answer a buyer who has ALREADY chosen the repair.
// =========================================================================

function CheckGlyph() {
  return <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2.5 7.5l3 3 6-7" /></svg>;
}

// ---------- A. Is this you? — simple symptom confirm ---------------------
// No board-level / component talk. The job is to CONFIRM they're in the
// right place, not re-open the diagnosis.
function SymptomConfirm({ price = 699 }) {
  const symptoms = [
    "Cracked or shattered glass",
    "Lines, bars, or flickering across the display",
    "Black patches or clusters of dead pixels",
    "Dim, patchy, or uneven backlight",
    "Colour bleeding, ghosting, or burn-in",
    "Glass lifting or no longer sitting flush",
  ];
  return (
    <section className="journey symptom" id="symptoms">
      <div className="container">
        <div className="journey-stage"><span className="journey-stage-num">01</span> Is this you?</div>
        <div className="symptom-grid">
          <div className="symptom-text">
            <h2>Cracked, dark, or lines on the display? That's this repair.</h2>
            <p className="journey-lede">
              If your MacBook Pro 16" is showing any of these, you're on the right page. One fixed price,
              a genuine Apple display, most machines back with you inside 48 hours.
            </p>
            <div className="symptom-verdict">
              <div className="symptom-verdict-key">Your repair</div>
              <div className="symptom-verdict-val">Screen replacement · <strong>£{price}</strong></div>
              <div className="symptom-verdict-foot">
                Not certain it's the screen? We diagnose every Mac free before any work starts — you'll
                never pay for a part you don't need.
              </div>
            </div>
          </div>
          <ul className="symptom-list">
            {symptoms.map(s => (
              <li key={s}><span className="symptom-tick"><CheckGlyph /></span>{s}</li>
            ))}
          </ul>
        </div>
      </div>
...
// ---------- B. What's in your price — itemised, positive ----------------
function PriceBreakdown({ price = 699 }) {
  const rows = [
    { label: "Genuine Apple display assembly", note: "A2991 · pulled OEM panel, not aftermarket" },
    { label: "True Tone & colour calibration", note: "Matched to a reference colorimeter" },
    { label: "30-point quality assurance", note: "Brightness, dimming zones, camera, Wi-Fi, ports" },
    { label: "2-year warranty", note: "Parts, labour, and any re-work" },
    { label: "Insured return or collection", note: "Walk-in, courier, or pre-paid mail-in" },
  ];
  return (
    <section className="journey journey-tight pricebreak" id="price">
      <div className="container">
        <div className="pb-grid">
          <div className="pb-text">
            <div className="journey-stage"><span className="journey-stage-num">02</span> What's in your price</div>
            <h2>£{price}. And that's the price.</h2>
            <p>
              No diagnostic fee bolted on at the end. No "we found something else" phone call to push the
              number up. The figure we quote is the figure you pay — everything on the right is already in it.
            </p>
            <p style={{ color: "#171717", fontWeight: 500 }}>
              If we open your Mac and the fix turns out simpler, the price comes <em>down</em>, not up. Either
              way you'll hear from us before we proceed.
            </p>
          </div>
          <div className="pb-card">
            <div className="pb-card-head">
              <span className="pb-card-head-label">Screen repair · MacBook Pro 16" M3</span>
            </div>
            {rows.map(r => (
              <div className="pb-row" key={r.label}>
                <div className="pb-row-main">
                  <span className="pb-row-label">{r.label}</span>
                  <span className="pb-row-note">{r.note}</span>
                </div>
                <span className="pb-row-inc"><span className="pb-row-inc-tick"><CheckGlyph /></span>Included</span>
              </div>
            ))}
            <div className="pb-total">
              <span className="pb-total-label">Total, fixed</span>
              <span className="pb-total-val">£{price}</span>
            </div>
          </div>
        </div>
      </div>
...
// ---------- C. Genuine parts, calibrated — quiet, positive --------------
// Reframe of the old "why shops can't" combat. No VS table, no punching down.
function GenuineParts() {
  const points = [
    {
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"><rect x="3" y="4" width="18" height="12" rx="1.5" /><path d="M9 20h6M12 16v4" /></svg>,
      t: "Original Apple display",
      b: "Pulled from first-party stock and matched to your exact model — never an aftermarket compatible panel.",
    },
    {
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3" /><path d="M12 3v3M12 18v3M3 12h3M18 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M18.4 5.6l-2.1 2.1M7.7 16.3l-2.1 2.1" /></svg>,
      t: "Calibrated in-house",
      b: "Flashed against a reference colorimeter so True Tone, full brightness, and colour accuracy survive the swap.",
    },
    {
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"><path d="M12 3l7 2.5v6c0 4.2-3 7.5-7 9-4-1.5-7-4.8-7-9v-6L12 3z" /><path d="M9 12l2.2 2.2L15.5 10" /></svg>,
      t: "No nasty surprises later",
      b: "Aftermarket panels throw the “Unknown Part” warning, drop True Tone, and fade within months. We don't fit them — which is why the 2-year warranty actually holds.",
    },
  ];
  return (
    <section className="journey genuine" id="parts">
      <div className="container">
        <div className="gp-grid">
          <div className="gp-text">
            <div className="journey-stage"><span className="journey-stage-num">·</span> Parts & calibration</div>
            <h2>Genuine parts, calibrated properly. So it just works.</h2>
            <p>
              A screen is only as good as the panel and the calibration behind it. We fit the same display
              Apple would, then spend the extra bench time matching it to a reference — so the repair is
              invisible the moment you open the lid.
            </p>
          </div>
          <div className="gp-points">
            {points.map(p => (
              <div className="gp-point" key={p.t}>
                <div className="gp-point-icon">{p.icon}</div>
                <div className="gp-point-body">
                  <h3>{p.t}</h3>
                  <p>{p.b}</p>
                </div>
              </div>
...
// ---------- D. Three ways in — logistics for a committed buyer ----------
function ThreeWaysIn() {
  const paths = [
    {
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 21V9l8-5 8 5v12" /><path d="M9 21v-6h6v6" /></svg>,
      title: "Walk in",
      meta: "Free · Mon–Fri · No appointment",
      body: "12 Margaret Street, Fitzrovia W1W 8JQ — opposite The London Palladium. Free diagnosis and a written quote while you wait.",
    },
    {
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 6h11v9H1zM12 9h5l4 3v3h-9" /><circle cx="6" cy="18" r="1.6" /><circle cx="17" cy="18" r="1.6" /></svg>,
      title: "Same-day courier",
      meta: "£20 · London · Insured",
      body: "We collect from your home or office and return it fully insured — often the very next working day.",
    },
    {
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="1.5" /><path d="M3 7l9 6 9-6" /></svg>,
      title: "Free mail-in",
      meta: "£0 · UK-wide · Pre-paid",
      body: "We send a pre-paid, insured label. Post it from anywhere in the UK and track your Mac the whole way.",
    },
  ];
  return (
    <section className="journey threeways" id="access">
      <div className="container">
        <div className="journey-stage"><span className="journey-stage-num">·</span> Getting it to us</div>
        <h2>Three ways in. One standard of repair.</h2>
        <p className="journey-lede">
          However your Mac reaches the bench, it's the same genuine-parts repair, the same calibration,
          and the same 2-year warranty.
        </p>
        <div className="twi-grid">
          {paths.map(p => (
            <div className="twi-card" key={p.title}>
              <div className="twi-icon">{p.icon}</div>
              <div className="twi-title">{p.title}</div>
              <div className="twi-meta">{p.meta}</div>
              <div className="twi-body">{p.body}</div>
            </div>
          ))}
        </div>
      </div>
...
Object.assign(window, {
  ...
  SymptomConfirm, PriceBreakdown, GenuineParts, ThreeWaysIn
});